export const meta = {
  name: 'verify-alignment-ru',
  description: 'Check whether each Russian segment is a translation of its aligned Italian segment (yes/partial/no); one Sonnet subagent per shard writes its own verdict file',
  phases: [{ title: 'Verify', detail: 'one subagent per 60-record shard checking segment alignment' }],
}

// args is an array of shard indices to run, e.g. [0, 1] for the pilot or 0..29 for the
...
function prompt(index) {
  const nn = pad(index)
  const shardPath = `${BASE}/data/verify_ru/shards/shard_${nn}.jsonl`
  const verdictPath = `${BASE}/data/verify_ru/verdicts/verdict_${nn}.jsonl`
  return `You are checking the alignment of an Italian–Russian parallel corpus for a
linguistics project. This is a data-quality pass: we want to confirm that each
Russian passage really is the translation of the Italian passage it was aligned to.

Read the JSONL file at ${shardPath}. Each line is one record:
  { "row_id", "idiom_it", "it_segment", "ru_segment" }
...
ru_segment is the passage from a published Russian translation (1936, tr. I. I.
Schitz; or 1999, tr. Н. Георгиевская & А. Эфрос) that was aligned to it.

For each record, judge ONE thing: is ru_segment a translation of it_segment?
  - "yes"     — ru_segment renders substantially all of it_segment; they are the
  ...
  - "partial" — clearly related and overlapping, but the Russian translates only
                part of the Italian, or carries substantial material the Italian
                does not (or vice versa) beyond normal latitude.
  - "no"      — ru_segment is not a translation of it_segment at all; they are
...
every row_id present exactly once. Each line:
  { "row_id": <copied from input>,
    ...
    "note": <for "partial"/"no": a one-clause explanation, e.g. "Russian continues
             past the Italian" or "unrelated passage"; for "yes": ""> }

Rules:
  - Do not skip, merge, reorder, or invent records. One output line per input line.
  - Never quote or re-type Russian text in your output beyond the fields above; the
    note must be in English. Do NOT normalise Russian spelling anywhere (see below).
  - Write ONLY the JSONL file. Do not print the verdicts back to me.

RUSSIAN ORTHOGRAPHY — IMPORTANT: the two editions follow opposite ё/е conventions
(1936 writes "еще", 1999 writes "ещё"). Both are correct for their edition. Never
"correct", modernise, or normalise ё↔е or any other character when reading or
...
const summaries = await parallel(
  shardIndices.map((index) => () =>
    agent(prompt(index), {
      label: `verify-ru:shard_${pad(index)}`,
      phase: 'Verify',
      model: 'sonnet',
